...
export function Navbar() {
    ...
    return (
        <nav className="flex items-center justify-between w-full px-6 py-4" style={{ background: 'var(--primary)', color: 'var(--primary-foreground)' }}>
            {/* Logo */}
            ...
            <div className="flex items-center gap-6">
                <>
                    <Link href="/" className="text-sm font-medium nav-link-custom">
                        Accueil
                    </Link>
                    <Link href="/books" className="text-sm font-medium nav-link-custom">
                        Livres
                    </Link>
                    <Link href="/writers" className="text-sm font-medium nav-link-custom">
                        Écrivains
                    </Link>
                    <Link href="/events" className="text-sm font-medium nav-link-custom">
                        Événements
                    </Link>
                    {isAuthenticated && (
                        <>
                            <Link href="/books/create" className="text-sm font-medium nav-link-custom">
                                Publier un livre
                            </Link>
                            <Link href="/events/create" className="text-sm font-medium nav-link-custom">
                                Créer un évènement
                            </Link>
                        </>
                    )}
            ...
            <div className="flex items-center gap-4">
                <Button
                    variant="ghost"
                    size="sm"
                    style={{ background: 'var(--primary)', color: 'var(--primary-foreground)', border: 'none', transition: 'background 0.2s, color 0.2s' }}
                    onMouseOver={e => { e.currentTarget.style.background = 'var(--card)'; e.currentTarget.style.color = 'var(--primary)'; }}
                    onMouseOut={e => { e.currentTarget.style.background = 'var(--primary)'; e.currentTarget.style.color = 'var(--primary-foreground)'; }}
                >
                    <Search className="h-4 w-4" style={{ color: 'inherit', transition: 'color 0.2s' }} />
                </Button>
                <DropdownMenu>
                ...
                {authLoading ? (
                    ...
                    <Button
                        ...
                        onMouseOver={e => {
                            ...
                            const icon = e.currentTarget.querySelector('.connexion-icon');
                        ...
                        onMouseOut={e => {
                            ...
                            const icon = e.currentTarget.querySelector('.connexion-icon');
                            if (icon) icon.style.color = 'var(--primary-foreground)';
                        }}
                        onClick={() => {
                    ...
                    >
                        <UserIcon className="h-4 w-4 mr-2 connexion-icon" style={{ color: 'var(--primary-foreground)', transition: 'color 0.2s' }} />
                        Connexion
                    </Button>
                )}
            </div>
